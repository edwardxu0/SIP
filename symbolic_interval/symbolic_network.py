...
class Interval_network(nn.Module):
        ...
        for layer in model:
            if isinstance(layer, nn.Linear):
                ...
                if last_layer and c is not None:
                    ...
                    wc_matrix = None
                self.net_.append(
                    Interval_Dense(layer, first_layer, wc_matrix=wc_matrix)
                )
                first_layer = False
            if isinstance(layer, nn.ReLU):
                self.net_.append(Interval_ReLU(layer))
            if isinstance(layer, nn.Conv2d):
                self.net_.append(Interval_Conv2d(layer, first_layer))
                first_layer = False
            if "Flatten" in (str(layer.__class__.__name__)):
                self.net_.append(Interval_Flatten())
            if "Vlayer" in (str(layer.__class__.__name__)):
                self.net_.append(Interval_Vlayer(layer))
            if "bn" in (str(layer.__class__.__name__)):
                self.net_.append(Interval_BN(layer))
	...
	* :attr:`ix` is the input fore each layer. If ix is a naive
    ...
    def forward(self, ix, sequential=False):
        if sequential:
            return self.net(ix)
        else:
            for layer in self.net_:
                ix = layer(ix)
                cix = copy.copy(ix)
                cix = cix.concretize()
                self.intermediate_ix += [cix]
            return ix
        """
    ...
    def forward(self, ix):
        ...
        if isinstance(ix, mix_interval):
            ...
            if m != 0:
                ...
                error_row = error_row.scatter_(
                    1, appr_ind[:, 1, None], appr_err[appr_condition][:, None]
                )
            ...
            ix.nc, ix.ne = (ix.nl + ix.nu) / 2.0, (ix.nu - ix.nl) / 2.0
            return ix
        ...
        if isinstance(ix, Inverse_interval):
            ...
            if ix.use_cuda:
                mask = mask * (
                    (upper > 0).type(torch.Tensor).cuda(device=ix.c.get_device())
                )
            else:
...
class Interval_Flatten(nn.Module):
    ...
    def forward(self, ix):
        if (
            isinstance(ix, Symbolic_interval)
            or isinstance(ix, Symbolic_interval_proj1)
            or isinstance(ix, Symbolic_interval_proj2)
            or isinstance(ix, Center_symbolic_interval)
        ):
            ix.extend()
            return ix
...
class Interval_Bound(nn.Module):
    def __init__(
        ...
        if proj is not None:
            assert proj > 0, (
                "project dimension has to be larger than 0,"
                " please use naive bound propagation (proj=0)!"
            )
            assert isinstance(proj, int), "project dimension has to" " be integer!"
    ...
    def forward(self, X, y):
        ...
        if self.worst_case:
            c = torch.eye(out_features).type_as(X)[y].unsqueeze(1) - torch.eye(
                out_features
        ...
        if self.method == "sym":
            ...
            if self.proj is None:
                if self.norm == "linf":
                    ...
                    ix = Symbolic_interval(
                        X, X, self.epsilon, norm="l2", use_cuda=self.use_cuda
                    )
                elif self.norm == "l1":
                    ix = Symbolic_interval(
                        X, X, self.epsilon, norm="l1", use_cuda=self.use_cuda
                    )
            else:
        ...
        ix = inet(ix)
        # print(inet.intermediate_ix)
        for l in inet.intermediate_ix:
            print(l.l.shape)
            print(l.l[0].shape)
        # print("u", ix.u, ix.u.shape)
        print("l", ix.l[0], ix.l.shape)
...
def naive_interval_analyze(
    net, epsilon, X, y, use_cuda=True, parallel=False, norm="linf"
):
    ...
    if parallel:
        wc = nn.DataParallel(
            Interval_Bound(net, epsilon, method="naive", use_cuda=use_cuda, norm=norm)
        )(X, y)
    else:
        wc = Interval_Bound(net, epsilon, method="naive", use_cuda=use_cuda, norm=norm)(
            X, y
        )
...
def inverse_interval_analyze(net, epsilon, X, y, use_cuda=True, parallel=False):
    ...
    if parallel:
        wc = nn.DataParallel(
            Interval_Bound(net, epsilon, method="inverse", use_cuda=use_cuda, norm=norm)
        )(X, y)
    else:
        wc = Interval_Bound(
            net, epsilon, method="inverse", use_cuda=use_cuda, norm=norm
        )(X, y)
...
def center_symbolic_interval_analyze(net, epsilon, X, y, use_cuda=True, parallel=False):
    ...
    if parallel:
        wc = nn.DataParallel(
            Interval_Bound(net, epsilon, method="center_sym", use_cuda=use_cuda)
        )(X, y)
    else:
...
def sym_interval_analyze(
    net, epsilon, X, y, use_cuda=True, parallel=False, proj=None, norm="linf"
):
    if parallel:
        wc = nn.DataParallel(
            Interval_Bound(
                net, epsilon, method="sym", proj=proj, use_cuda=use_cuda, norm=norm
            )
        )(X, y)
    else:
        wc = Interval_Bound(
...
def mix_interval_analyze(
    net, epsilon, X, y, use_cuda=True, parallel=False, proj=None, norm="linf"
):
    if parallel:
        wc = nn.DataParallel(
            Interval_Bound(
                net, epsilon, method="mix", proj=proj, use_cuda=use_cuda, norm=norm
            )
        )(X, y)
    else:
        wc = Interval_Bound(
...
def gen_interval_analyze(
    ...
    if parallel:
        wc = nn.DataParallel(
            Interval_Bound(
                net, epsilon, method="gen", proj=proj, use_cuda=use_cuda, norm=norm
            )
        )(X, y)
    else:
        wc = Interval_Bound(
